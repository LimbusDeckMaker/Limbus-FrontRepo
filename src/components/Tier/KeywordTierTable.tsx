...
const KeywordTierTable = () => {
  ...
  // 출혈: 100 약상, 119 적슈, 102 선장마엘, 27 퀴히스, 30 콩루, 10 쥐파, 6 중돈, 65 료슈, 20 갈그렉, 72 쥐싱,
  //      64 흑슈, 34 갈루, 101 약티스, 57 흑로쟈, 29 흑루, 82 피상, 90 흑그렉, 48 중뫼
  const S = [
    100, 119, 102, 27, 30, 10, 6, 65, 20, 72, 64, 34, 101, 57, 29, 82, 90, 48,
  ];

  // 화상: 103 런싱, 40 리우마엘, 91 리쟈, 83 마티스, 92 리슈
  const A = [103, 40, 91, 83, 92];

  // 진동: 13 후파우, 53 어티스, 110 탐루, 122 츠이스, 93 외히스, 113 티로쟈, 112 티돈, 111 유슈, 37 대리마엘
  const B = [13, 53, 110, 122, 93, 113, 112, 111, 37];

  // 파열: 121 제로쟈, 12 세파우, 32 k루, 44 떱르소, 84 초돈, 26 셉히스, 52 셉티스, 95 데뫼, 18 장그렉
  const C = [121, 12, 32, 44, 84, 26, 52, 95, 18];

  // 침잠: 61 디로쟈, 120 죽나상, 114 디뫼, 85 디루, 94 치티스, 41 해녀마엘, 79 동상, 96 퐁그렉
  const D = [61, 120, 118, 114, 85, 94, 41, 79, 96];
