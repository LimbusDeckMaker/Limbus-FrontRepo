...
const TierTable = () => {
  ...
  // S+: 100 약상, 72 쥐싱, 102 선장마엘, 61 디로쟈 120 죽나상 119 적슈 13 후파우
  const S = [100, 72, 102, 61, 120, 119, 13];

  // S: 27 퀴히스, 75 섕싱, 98 넬슈, 116 멀파우, 118 마히스, 85 디루, 23 r히스, 103 새벽싱
  const A = [27, 75, 98, 116, 118, 85, 23, 113, 103];

  // A: 114 디뫼, 32 k루, 20 갈그렉, 94 치티스, 96 퐁그렉, 30 콩루, 83 마티스
  const B = [114, 32, 20, 94, 96, 30, 83];

  // B: 81 떱상, 66 떱슈, 3 떱돈, 115 떱티스, 6 중돈, 10 쥐파, 79 동상
  const C = [81, 66, 3, 115, 6, 10, 79];

  // C: 91 리쟈, 40 리우마엘, 12 세파우, 110 탐루
  const D = [91, 40, 12, 110];
